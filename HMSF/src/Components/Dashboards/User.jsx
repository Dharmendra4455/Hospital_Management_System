    ...
    <Welcomebox/>
     ...
     <div className="doc_Chart  flex justify-baseline gap-4 m-2 w-full" >
      <div className='h-52 overflow-auto'>
           ...
           {/* Appointment_Update Card Section */}
                ...
                AppointmentUpdatedata.update.map((item, id) => {
                  return (
                    <div key={id} className={                  
                        item.status === 'Progress' ? 'bg-yellow-400 cards h-fit w-68 mr-2 hover:bg-zinc-300 rounded-md pt-4' :
                        item.status === 'Discharged' ? 'bg-green-700 cards h-fit w-68 mr-2  hover:bg-zinc-300 rounded-md pt-4' :
                        item.status === 'Good' ? 'bg-green-400 cards h-fit w-68 mr-2  hover:bg-zinc-300 rounded-md pt-4' :
                        item.status === 'Critical' ?  'bg-red-400 cards h-fit w-68 mr-2  hover:bg-zinc-300 rounded-md pt-4':
                                                 'bg-blue-700 cards h-fit w-68 mr-2  hover:bg-zinc-300 rounded-md pt-4'
                        }
                         >
                      ...
                      <div className=" text-black font-bold mt-2 pt-1.5 pb-1.5 flex justify-between gap-2 border-2 border-zinc-400 rounded-sm text-[14px] mr-1 ml-1 pr-1 pl-1 ">
                        ...
                        <div className=" text-[13px]  datetime flex justify-end gap-1 font-bold">
                          <div className="date">{item.updatedAt.split('T')[0]}{"  "}</div>
                          <div className="time">{'  '+ new Date(item.updatedAt).toLocaleTimeString()}</div>
                        </div>
                      </div>
                      <div 
