...
const Appointmentform = (props) => {
  const[firstname,setfirstname]=useState("Dharmendra")
  ...
  const[email,setemail]=useState("dharmendrapatel123@gmail.com")
...
// console.log(firstname+dob+email+gender+contact+address+city+state+department+salary)    
  const doctor_detail={firstname,lastname,dob,email,password,gender,contact,address,city,state,department,salary}
  // alert(doctor_detail)
  const submithandler=()=>{
    if(props.data==='Admin'){
    if(firstname&&dob&&email&&password&&gender&&contact&&address&&city&&state&&department&&salary)
     {  
      ...
      if(password===conformpassword){
       ...
       .then(response => {
           ...
           alert(response.data.message || 'Doctor created!');
           setfirstname("")
           setlastname("")
           setdob("")
           setemail("")
        ...
        alert("Conform password mismatch!!") 
      }}
      else{
     ...
     const appointment_detail={firstname,lastname,dob,email,gender,contact,address,city,state,department,doctor,status,appointmentdate,alloteddate,allotedtime,description}
      if(contact.length==10){
       ...
       .then(response => {
           console.log('API response:', response.data);
           alert(response.data.message || 'Appointment created!');
           setfirstname("")
           setlastname("")
           setdob("")
           setemail("")
 ...
 <button onClick={closehandler} className="btn btn-sm btn-circle btn-ghost absolute right-2 top-2 ">✕</button>
    ...
    <input className="first border p-1 rounded" required value={props.data==="User"?"Dharmendra":firstname}  placeholder={props.data==="User"?"":"firstname"} onChange={(e)=>{ setfirstname(e.target.value)}}></input>
    <input className="lastname border p-1 ml-5 rounded" value={props.data==="User"?"Patel":lastname} placeholder={props.data==="User"?"":"lastname"}  onChange={(e)=>{setlastname(e.target.value)}}></input>
    </label>
    ...
    <input className="first border p-1 rounded w-[400px]" required value={props.data==="User"?"Dharmendra123@gmail.com":email} placeholder={props.data==="User"?"":"xyz@gmail.com"} onChange={(e)=>{setemail(e.target.value)}} ></input>
   
   {props.data==='Admin'? 
    <div className="password mt-2">
    <label className='text-md mt-5'>Password<sup className='text-red-600 text-xl '>*</sup></label>
    <label className=" flex ">
    <input className="first border p-1 rounded" required type='password' value={password} onChange={(e)=>{setpassword(e.target.value)
